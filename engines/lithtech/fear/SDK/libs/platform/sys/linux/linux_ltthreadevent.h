...
static const uint32 k_nNanosPerSecond = 1000000000; 
...
LTThreadEventBlockResult CLinux_LTThreadEvent<eThreadEventType>::Block(uint32 nTimeout /* = INFINITE */)
{
	...
	{
		LTTimeUtils::GetCurrentTime(timeNow);
		
		timeToWait.tv_sec = timeNow.tv_sec;
		timeToWait.tv_nsec = (timeNow.tv_usec * 1000) + (nTimeout * 1000000);
		
		// check to see if we've exceeded the number of nanos in a second
		while (timeToWait.tv_nsec >= k_nNanosPerSecond)
		{
			// set the nanosecond portion to the remainder
			timeToWait.tv_nsec -= k_nNanosPerSecond;
