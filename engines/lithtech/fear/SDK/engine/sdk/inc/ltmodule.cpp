...
void CInterfaceDatabase::TransferFrom(CInterfaceDatabase *other) {
    ...
    for (int32 i = other->interfaces->Num() - 1; i >= 0; i--) {
        ...
        if (our_mgr == NULL) {
            //make a mgr
            LT_MEM_TRACK_ALLOC(our_mgr = new CInterfaceNameMgr(other_mgr->InterfaceName(), other_mgr->InterfaceVersion()), LT_MEM_TYPE_INTERFACEDB);
...
{
    //copy the name.
    LT_MEM_TRACK_ALLOC(this->name = LTStrDup(name), LT_MEM_TYPE_INTERFACEDB);
    ...
    //allocate our arrays.
	typedef database_array<IBase, const char *> TAllocType;

    LT_MEM_TRACK_ALLOC(interfaces = new TAllocType, LT_MEM_TYPE_INTERFACEDB);
    LT_MEM_TRACK_ALLOC(holders = new database_list<CAPIHolderBase>, LT_MEM_TYPE_INTERFACEDB);
}
...
//****************************************************************************************
    ...
    database_array<C, id_type>::database_array() {
        ...
        array_len = 1;
        LT_MEM_TRACK_ALLOC(items = new C *[array_len], LT_MEM_TYPE_INTERFACEDB);
    }
