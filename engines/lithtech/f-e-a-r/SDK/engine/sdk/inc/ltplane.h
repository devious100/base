...
//called to determine if this plane contains any NaN's
inline bool LTIsNaN(const LTPlane& Plane)
{
	return LTIsNaN(Plane.Normal()) || LTIsNaN(Plane.Dist());
}

//Serialization functions
inline void LTStream_Read(ILTInStream *pStream, LTPlane& plane)
{
	*pStream >> plane.m_Normal;
	*pStream >> plane.m_Dist;
}

inline void LTStream_Write(ILTOutStream *pStream, const LTPlane& plane)
{
	*pStream << plane.m_Normal;
	*pStream << plane.m_Dist;
}
