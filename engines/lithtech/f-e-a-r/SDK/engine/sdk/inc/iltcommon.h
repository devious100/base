...
*/
    virtual LTRESULT GetAttachmentTransform(HATTACHMENT hAttachment, 
        LTTransform &transform, bool bWorldSpace) = 0;
...
*/
	...
	virtual LTRESULT Parse(ConParseW *pParse) = 0;
...
Used for: Misc.  */
	...
	/*!
	\param hPoly The handle of the polygon.
	\param pIndex Pointer to dest where the polygon's occlusion index is written.

	\return \b LT_ERROR when no world is loaded or \b hPoly is
	invalid; otherwise, returns \b LT_OK.

	Get the occlusionIndex for a poly.

	Used for: Misc.  */
	virtual LTRESULT GetPolyOcclusionIndex(HPOLY hPoly, uint8 *pIndex)=0;
/*!
