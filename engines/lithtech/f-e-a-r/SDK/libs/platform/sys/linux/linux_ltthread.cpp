...
void CLinux_LTThread::Create(UserThreadFunction pUserThreadFunction, void* pArgument, uint32 nProcessorAffinity)
{
	LTASSERT(m_hThread == 0, "Attempt to create thread that has already been created");
	...
	LTASSERT(returnValue == 0, "Failed to create thread");
}

void CLinux_LTThread::WaitForExit()
{
	LTASSERT(m_hThread != 0, "Attempt to WaitForExit with invalid handle");
...
void* InternalThreadFunction(void* pArgument)
{
	...
	CLinux_LTThread::InternalThreadArgument* internalThreadArgument = (CLinux_LTThread::InternalThreadArgument*) pArgument;
