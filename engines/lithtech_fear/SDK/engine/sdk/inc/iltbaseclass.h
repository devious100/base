/*!
 ...
 PURPOSE : C++ Game base class interface
...
Engine Programming Guide for more information.
...
1) Load properties from a RESEXT_MODEL_PACKED (formerly .LTB and .DAT) file into
...
/*!
\param pMsg the ILTMessage_Read that contains the saved Special Effect Message.
\param dwParam user parameters (passed to ILTServer::SaveObjects)
\return Returns 1 (ignored).

Called by the engine when the object should serialize it's Special Effect Message.

This method should read each item from the saved Special Effect Message, pMsg, and 
write each item to a new message and call SetObjectSFXMessage().

Used For: Object.
*/
	virtual uint32 OnLoadSFXMessage				( ILTMessage_Read *pMsg, float fParam );


/*!
\param pMsg the ILTMessage_Write to contain the saved Special Effect Message.
\param dwParam user parameters (passed to ILTServer::SaveObjects)
\return Returns 1 (ignored).

Called by the engine when the object should serialize it's Special Effect Message.

This method should read each item from the current Special Effect Message, by calling GetObjectSFXMessage, and 
write each item to pMsg.

Used For: Object.
*/

	virtual uint32 OnSaveSFXMessage				( ILTMessage_Write *pMsg, float fParam );
...
/*!
Here are all the message IDs and structures that Engine.exe uses for object notifications.
*/

/*!
These are the engine message identifications ("MID")
and structures that Engine.exe sends to an object's \b EngineMessageFn.

\b EngineMessageFn and these messages will be deprecated in a future release of Engine.exe,
...
\b fData is the \b ANIMTRACKERID that indicates the animation tracker 
that triggered the string.
*/
...
/*!
Load and save your special effect message for a serialization.
\b pData is an \b HMESSAGEREAD or \b HMESSAGEWRITE.
\b fData is the \b dwParam passed to ServerDE::SaveObjects or ServerDE::RestoreObjects.
*/
	MID_LOADSPECIALEFFECTMESSAGE =      16,
	MID_SAVESPECIALEFFECTMESSAGE =      17,
};
