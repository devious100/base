...
*/
	...
	virtual LTRESULT SetSoundVolume(HLTSOUND hSound, uint8 nVolume, LTFLOAT fFadeTime=0) = 0;
	
	virtual LTRESULT GetSoundVolume(HLTSOUND hSound, uint8 &nVolume, LTFLOAT &fFadeTime) = 0;


	/*!
	\param  pSoundCaps               The struct to return the caps in.

	\return \b LT_OK.

	Gets the sound capabilities so that the options can be set accordingly..

	Used for: Audio/Profiles.
	*/

	virtual LTRESULT GetSoundCapabilities(SoundCapabilities* pSoundCaps) = 0;
	/*!
	\param  pSoundCaps               The sound caps the user desires (if they can be achieved).

	\return \b LT_OK.

	Sets the sound capabilities, if possible.

	Used for: Audio/Profiles.
	*/

	virtual LTRESULT SetSoundCapabilityPreferences(SoundCapabilities SoundCaps) = 0;

	virtual	bool	GetSoundInitError(char* pszSoundInitError, int32 nStringSize) = 0;
};
...
*/
	...
	/*!
	\param  pParam      Name of the filter.
	\return \b LT_ERROR when engine does not support filters, 
	...
	Used for: Audio.  
	*/
	virtual LTRESULT SetSoundFilterName(const char *pParam) = 0;
/*!
...
\param  bTeleport	true if the sound is to teleport (ie, not be considered to have a velocity)
...
*/
	...
	*/
	...
	/*!
	\return \b LT_ERROR when unable to stop; otherwise, returns \b LT_OK.

	Stops all sounds.
	if bReturningToFE is TRUE, then sounds marked as FE persistant sounds will
	not be stopped.

	Used for: Audio.
	*/
	virtual LTRESULT StopAllSounds(bool bReturningToFE ) = 0;

	/*!
	\return \b LT_ERROR when unable to stop; otherwise, returns \b LT_OK.

	Cleans up all sounds (useful for when loading a saved game).
	Used for: Audio.
	*/
	virtual LTRESULT RemoveAllUnusedSoundInstances(  ) = 0;
...
*/
	...
	/*!
	\param  mixer               New mixer settings.
	\param  OverrideTime		Overrides the fade-in/out time.. -1 to use default
	\param  bKillTempMixer		true if you only want to kill the temp mixer, not set a new mixer.

	\return \b LT_OK.

	Set the current sound mixer.  

	Used for: Audio.
	*/

		virtual LTRESULT	SetSoundMixer( SoundMixerInfo* mixer,
			int32 OverrideTime, int32 KillLayer ) = 0;

		/*!
		\param  pMainMix               New main mixer settings.

		\return \b LT_OK.

		Set the main sound mixer. This adjusts overall levels (ie for 
			SFX and music volume levels)

		Used for: Audio.
		*/

		virtual LTRESULT SetSoundMixerMainLevels(SoundMixerSliders* pMainMix) = 0;

		/*!
		\param  pChannelLimits               An array of channel limits. Should go up to MAX_MIXER_CHANNELS but can be less.
				nNumChannelLimits			The number of channel limits in pChannelLimits list.

		\return \b LT_OK.

		Set the limits for each mix channel for the number of sounds of that type
		that can be playing at one time. Use this to control sounds that can chew
		up channels, such as bullet impacts.

		Used for: Audio.
		*/

		virtual LTRESULT SetMixerChannelLimits(int32* pChannelLimits, int32 nNumChannelLimits) = 0;

		/*!

		\return \b LT_OK.

		Clears out all the temp sound mixers.

		Used for: Audio.
		*/

		virtual LTRESULT ClearTempSoundMixers() = 0;

		/*!
		\param  pszRegionName               Name of the region being deactivated..

		\return \b LT_OK.

		Deactives a sound streaming region so it can be unloaded later.

		Used for: Audio.
		*/

		virtual LTRESULT DeactivateSoundRegion(char* pszRegionName) = 0;
	};
