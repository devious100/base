...
public:
	...
	TRect2(const TVector2<T>& vMin, const TVector2<T>& vMax) :
		m_vMin(vMin),
		m_vMax(vMax)
	{}
	...
	void Init(const TVector2<T>& vMin, const TVector2<T>& vMax)
	{
		m_vMin = vMin;
		m_vMax = vMax;
	}
	...
	bool	Contains(const TVector2<T> pt) const
	{
	...
	const TVector2<T>&	GetTopLeft() const
	{
		return m_vMin;
	}

	const TVector2<T>&	GetBottomRight() const
	{
		return m_vMax;
	}

	const TVector2<T>	GetTopRight() const
	{
		return TVector2<T>(m_vMax.x, m_vMin.y);
	}

	const TVector2<T>	GetBottomLeft() const
	{
		return TVector2<T>(m_vMin.x, m_vMax.y);
	}
