...
surfcache_t     *D_SCAlloc (int width, int size)
{
	...
	size = (INT_PTR)&((surfcache_t *)0)->data[size];
	size = (size + 3) & ~3;
	if (size > sc_size)
...
// colect and free surfcache_t blocks until the rover block is large enough
	...
	// free another
		...
		new_cpp->size += sc_rover->size;
		new_cpp->next = sc_rover->next;
	}

// create a fragment out of any leftovers
	if (new_cpp->size - size > 256)
	{
		sc_rover = (surfcache_t *)( (byte *)new_cpp + size);
		sc_rover->size = new_cpp->size - size;
		sc_rover->next = new_cpp->next;
		sc_rover->width = 0;
		sc_rover->owner = NULL;
		new_cpp->next = sc_rover;
		new_cpp->size = size;
	}
	else
		sc_rover = new_cpp->next;
	
	new_cpp->width = width;
// DEBUG
	if (width > 0)
		new_cpp->height = (size - sizeof(*new_cpp) + sizeof(new_cpp->data)) / width;

	new_cpp->owner = NULL;              // should be set properly after return
