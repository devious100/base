...
char *Sys_ScanForCD (void)
{
	...
	for (drive[0] = 'c' ; drive[0] <= 'z' ; drive[0]++)
	{
		...
		sprintf_s (cddir, "%sinstall\\data", drive); // ***GREGS_VC9_PORT_MOD*** -- changed to safe version of sprintf
		sprintf_s (test, "%sinstall\\data\\quake2.exe", drive); // ***GREGS_VC9_PORT_MOD*** -- changed to safe version of sprintf
		/*f =*/ fopen_s(&f, test, "r"); // ***GREGS_VC9_PORT_MOD*** -- changed to safe version of fopen
		if (f)
		{
			fclose (f);
...
void Sys_ConsoleOutput (char *string)
{
	...
	WriteFile(houtput, string, (int)strlen(string), (LPDWORD)&dummy, NULL);
