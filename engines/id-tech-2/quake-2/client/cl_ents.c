...
// update radar extension
	...
	RadarType GetRadarType(entity_state_t* state);
	
	// update interval in ms
	#define UPDATE_RADAR_MS	500
#endif
...
void CL_ParseFrame (void)
{
	...
	if (cl.frame.valid)
	{
		...
		#if _MANAGED
		// update the radar
		if (cl_radarvisible && !cl_timedemo->value && &cl.frame)
			UpdateRadar(&cl.frame);
		#endif
	}
}
...
void UpdateRadar(frame_t *frame)
{
	...
	// store radar items in a STL vector list
...
// returns the radar type for the specified entity_state item
RadarType GetRadarType(entity_state_t* state)
{
	// unknown
	if (state->modelindex == 0)
		return RadarTypeUnknown;

	// player
	if (state->modelindex == 255)
		return (state->solid) ? RadarTypePlayer : RadarTypeDeadMonsters;

	// determine type from modelstring
	char* modelstring = cl.configstrings[CS_MODELS+(state->modelindex)];

	// dead
	if (strstr(modelstring, "dead") != NULL)
		return RadarTypeDeadMonsters;
	
	// monster
	if (strstr(modelstring, "monster") != NULL)
		return (state->solid) ? RadarTypeMonster : RadarTypeDeadMonsters;

	// health
	if (strstr(modelstring, "healing") != NULL)
		return RadarTypeHealth;

	// ammo, weapson, other
	if (strstr(modelstring, "items") != NULL)
		return RadarTypeItems;

	// ammo, weapson, other
	if (strstr(modelstring, "weapon") != NULL)
		return RadarTypeItems;
