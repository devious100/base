...
//			SelectPalette(hdcScreen, holdpal, FALSE);
	...
	else
	{
		...
		{
			if ( ( rval = sww_state.lpddsFrontBuffer->BltFast(
				0, 0, sww_state.lpddsOffScreenBuffer, &r, DDBLTFAST_WAIT ) ) == DDERR_SURFACELOST )
			{
				sww_state.lpddsFrontBuffer->Restore();
				
				sww_state.lpddsFrontBuffer->BltFast(
...
void Sys_MakeCodeWriteable (unsigned long startaddr, unsigned long length)
{
	...
	if (!VirtualProtect((LPVOID)(ULONG_PTR)startaddr, length, PAGE_READWRITE, &flOldProtect))
...
#else
unsigned long fpu_ceil_cw, fpu_chop_cw, fpu_full_cw, fpu_cw, fpu_pushed_cw;
unsigned long fpu_sp24_cw, fpu_sp24_ceil_cw;
