	...
	*/
	if ( currententity->backlerp == 0 )
	{
		if ( ccodes[0] == BBOX_TRIVIAL_ACCEPT )
			...
			return ( (ccodes[0] & ~BBOX_TRIVIAL_REJECT) != 0 ? true : false );
	}
	...
	if ( ( ccodes[0] | ccodes[1] ) == BBOX_TRIVIAL_ACCEPT )
		...
		return (( ccodes[0] | ccodes[1] ) & ~BBOX_TRIVIAL_REJECT) != 0 ? true : false;
}
...
//PGM
	...
	pfinalverts = (finalvert_t *)
			(((LONG_PTR)&finalverts[0] + CACHE_SIZE - 1) & ~(CACHE_SIZE - 1));
...
typedef struct spanpackage_t spanpackage_t;

void R_AliasDrawModel (void)
{
	extern void (*d_pdrawspans)(spanpackage_t *);
	extern void R_PolysetDrawSpans8_Opaque( spanpackage_t * );
	extern void R_PolysetDrawSpans8_33( spanpackage_t * );
	extern void R_PolysetDrawSpans8_66( spanpackage_t * );
	extern void R_PolysetDrawSpansConstant8_33( spanpackage_t * );
	extern void R_PolysetDrawSpansConstant8_66( spanpackage_t * );
