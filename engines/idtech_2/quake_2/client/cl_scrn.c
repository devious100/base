...
void SCR_CenterPrint (char *str)
{
	...
	strncpy_s(scr_centerstring, str, sizeof(scr_centerstring)-1); // ***GREGS_VC9_PORT_MOD*** -- changed to the safe version of CRT function
...
int entitycmpfnc( const entity_t *a, const entity_t *b )
{
	...
	*/
	if ( a->model == b->model )
	{
		return ( (INT_PTR) a->skin - (INT_PTR) b->skin );
	}
	else
	{
		return ( (INT_PTR) a->model - (INT_PTR) b->model );
	}
}
...
void SCR_ExecuteLayoutString (char *s)
{
	...
	while (s)
	{
		...
		{	// draw a ctf client block
			...
			sprintf_s(block, "%3d %3d %-12.12s", score, ping, ci->name); // ***GREGS_VC9_PORT_MOD*** -- changed to safe version of CRT function
