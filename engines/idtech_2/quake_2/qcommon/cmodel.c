...
void CMod_LoadSurfaces (lump_t *l)
{
	...
	for ( i=0 ; i<count ; i++, in++, out++)
	{
		strncpy_s (out->c.name, in->texture, sizeof(out->c.name)-1); // ***GREGS_VC9_PORT_MOD*** -- changed to safe version of CRT function
		strncpy_s (out->rname, in->texture, sizeof(out->rname)-1); // ***GREGS_VC9_PORT_MOD*** -- changed to safe version of CRT function
...
cmodel_t *CM_LoadMap (char *name, qboolean clientload, unsigned *checksum)
{
	...
	*checksum = last_checksum;
	...
	strcpy_s (map_name, name); // ***GREGS_VC9_PORT_MOD*** -- changed to safe version of CRT function
