...
#pragma warning (disable:4793) // ***GREGS_VC9_PORT_MOD*** -- shut up compiler warnings about functions being compiled as native
#pragma warning (disable:4611) // ***GREGS_VC9_PORT_MOD*** -- shut up compiler warnings about _setjmp and C++ object destruction being non-portable 
                               //                             (nothing we can do about it since the object being used is in a Microsoft header file)
...
void Com_Printf (char *fmt, ...)
{
	...
	va_start (argptr,fmt);
	vsprintf_s (msg,fmt,argptr); // ***GREGS_VC9_PORT_MOD*** -- changed to safe version of vsprintf
	va_end (argptr);
	...
	if (rd_target)
	{
		...
		strcat_s(rd_buffer, strlen(msg), msg); // ***GREGS_VC9_PORT_MOD*** -- changed to safe version of CRT function
		return;
	}
	...
	if (logfile_active && logfile_active->value)
	{
		...
		if (!logfile)
		{
			...
			if (logfile_active->value > 2)
				/*logfile = */fopen_s(&logfile, name, "a"); // ***GREGS_VC9_PORT_MOD*** -- changed to safe version of fopen
			else
				/*logfile = */fopen_s(&logfile, name, "w"); // ***GREGS_VC9_PORT_MOD*** -- changed to safe version of fopen
		}
		if (logfile)
...
char *CopyString (char *in)
{
	...
	strcpy_s(out, (int)strlen(in)+1, in); // ***GREGS_VC9_PORT_MOD*** -- changed to safe version of CRT function
	return out;
}
...
void Qcommon_Init (int argc, char **argv)
{
	...
	s = va("%4.2f %s %s %s %s", VERSION, CPUSTRING, __DATE__, __TIME__, BUILDSTRING);
...
void Qcommon_Frame (int msec)
{
	...
	if ( log_stats->modified )
	{
		...
		if ( log_stats->value )
		{
			...
			/*log_stats_file =*/ fopen_s(&log_stats_file, "stats.log", "w" ); // ***GREGS_VC9_PORT_MOD*** -- changed to safe version of fopen
			if ( log_stats_file )
