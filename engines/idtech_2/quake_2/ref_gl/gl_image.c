...
void GL_SelectTexture( GLenum texture )
{
	...
	if ( texture == (unsigned int)GL_TEXTURE0 )
	{
		tmu = 0;
	}
	else
...
void GL_TexEnv( GLenum mode )
{
	...
	if ( mode != (unsigned int)lastmodes[gl_state.currenttmu] )
	{
...
void GL_MBind( GLenum target, int texnum )
{
	...
	if ( target == (unsigned int)GL_TEXTURE0 )
	{
...
void GL_FreeUnusedImages (void)
{
	...
	for (i=0, image=gltextures ; i<numgltextures ; i++, image++)
	{
		...
		// free it
		qglDeleteTextures ((GLsizei)1, (const GLuint*)&image->texnum);
...
void	GL_InitImages (void)
{
	...
	if ( qglColorTableEXT )
	{
		ri.FS_LoadFile((char*)"pics/16to8.dat", (void**)&gl_state.d_16to8table );
	...
	for ( i = 0; i < 256; i++ )
	{
		...
		else
		{
			...
			inf = 255 * pow ((float)((i+0.5)/255.5) , (float)g ) + 0.5;
			if (inf < 0)
				inf = 0;
			if (inf > 255)
				inf = 255;
